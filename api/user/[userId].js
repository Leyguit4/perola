...
module.exports = async function handler(req, res) {
  ...
  if (!botToken) return res.status(500).json({ error: "BOT_TOKEN não configurado" });
  ...
  try {
    ...
    if (memberRes.ok) {
      ...
      if (userRes.ok) {
        ...
        if (user.avatar) avatarUrl = `https://cdn.discordapp.com/avatars/${userId}/${user.avatar}.png?size=256`;
      }
    }
  } catch (err) {
